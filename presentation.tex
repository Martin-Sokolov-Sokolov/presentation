...
\begin{frame}{FVS-4CL on a 5x5 grid}
  ...
  \begin{tikzpicture}[scale=1.1, every node/.style={circle, draw, minimum size=6mm},
                      rednode/.style={circle, draw=red, fill=red!30, thick, minimum size=6mm},
                      font=\small]
...
\foreach \x in {0,1,2,3,4} {
  ...
  \footnotesize
  \begin{definition}[Feedback Vertex Set with Four Cycle Length (\textbf{FVS-4CL})]
...
\begin{frame}{Approximation Bound via Subgraph Solutions}
    \begin{itemize}
      \item Let \( S_{i,j} \) be the minimum unweighted feedback vertex set (FVS-4CL)  
        for subgraph \( G_{i,j} \), computed using Algorithm~\ref{alg:dp}.

      \item Let \( F \equiv \text{OPT} \) be the optimum solution for the full graph \( G \),  
        and define \( F_{i,j} := F \cap V(G_{i,j}) \), the restriction of \( F \) to subgraph \( G_{i,j} \).

      \vspace{1em}
      \item We obtain the following bound on the total solution size:
      \[
      \underbrace{|S_{i^*}| \leq \sum\limits_{j} |S_{i^*, j}|}_{\text{by Lemma~\ref{lemma:opt-unweighted-fvs-4cl} and Result~\ref{result-two}}}
      \underbrace{\leq \sum\limits_{j} |F_{i^*, j}|}_{\text{by Lemma~\ref{lemma-two}}}
      \underbrace{\leq \left(1 + \frac{2}{\ell}\right) \cdot |F|}_{\text{by Lemma~\ref{lemma-three}}}
      = (1 + 2\epsilon)\cdot |F|
      \]
    \end{itemize}
...
\begin{tabular}{ccc}
    ...
    \begin{minipage}{0.3\textwidth}
        ...
        \begin{tikzpicture}[scale=0.5]
            ...
            \draw[blue, thick] (7) circle(0.3);
        \end{tikzpicture}
        \captionof{figure}{$S_2 \cap N^2_G[J]$}
    \end{minipage}
    &
    \begin{minipage}{0.3\textwidth}
        \centering
...
\begin{frame}{Lemma: FVS-4CL is 2-stitchable}
  \begin{lemma}[FVS-4CL is 2-stitchable]
